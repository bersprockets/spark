...
class GeneratorFunctionSuite extends QueryTest with SharedSparkSession {
  ...
  test("SPARK-39061: inline should handle null struct (with codegen)") {
    withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> "true") {
      testInlineNullStruct
    }
  }

  test("SPARK-39061: inline should handle null struct (without codegen)") {
